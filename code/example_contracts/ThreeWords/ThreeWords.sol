    ...
    ERC721
{
    ...
    // a PhraseID is useful for storing a "reference" to the three words for that token. The PhraseID is unique--a hash of the three words. The PhraseID can be used to efficiently check if a phrase has already been minted, in order to avoid duplicates.
